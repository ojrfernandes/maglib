...
int main(int argc, char *argv[]) {
  ...
  // create vector of input.nSections angular coordinates from input.phi_0 to
  // input.phi_1 (deg)
  ...
  if (input.nSections > 1) {
    ...
    std::cout << "\nPoincaré section defined at Phi (deg) = " << input.Phi
              << std::endl;
  }
  ...
  for (const auto &phi : poincare_sections) {
    ...
    if (base_path.size() >= 4 &&
        base_path.substr(base_path.size() - 4) == ".dat")
      base_path = base_path.substr(0, base_path.size() - 4);
    std::string section_file =
        base_path + "_" + std::to_string(static_cast<int>(phi)) + ".dat";
    {
      std::ifstream test_file(section_file);
      if (test_file.is_open()) {
        std::cerr << "Warning: File " << section_file
                  << " already exists. Saving to a new file to avoid "
                     "overwriting your data."
                  << std::endl;
        section_file = base_path + "_" + std::to_string(static_cast<int>(phi)) +
                       "_new.dat";
      }
    }
    ...
    manifold.configure(input.epsilon, input.h_deriv, input.n_tol,
                       input.max_iter, input.precision, input.max_insertions);
    ...
    std::cout << "\nManifold successfully computed for Poincaré section phi = "
              << static_cast<int>(phi) << "\n"
