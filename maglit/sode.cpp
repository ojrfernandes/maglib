...
int sode::evolve(double *x, double *t, double t_end, int chg_mon, void *aux) {
    ...
    if (status != SODE_OK) {
        h = 0.5 * h;
        if (verb)
            printf("bad function at t = %f\n", *t);
        return status; // return SODE_BAD_FUNC
    }
    if (verb)
        printf("sode::evolve: 1\n");
    ...
    *t = *t + h;
    ...
    if (!bisecting) { // regular step-size control
        double r;
        if (err > 0.5 * tol_sol)
            r = damp * pow(tol_sol / err, 1.0 / q);
        else
            r = damp * pow(tol_sol / err, 1.0 / (q + 1.0));
        ...
        if (fabs(h) > h_max)
            h = h_max * fabs(h) / h;
        if (verb)
            printf("regular step control h: %.3f, r: %.3e\n", h, r);
    }
    if (verb)
        printf("sode::evolve: 5\n");
    if (fabs(*t - t_end) < tol_end)
        return SODE_SUCCESS_TIME;
    else if (fabs(*t - t_end) < h)
        h = t_end - *t;
...
void sode::alloc_RK78() {
    ...
    b1 = new double[s];
    b1[0] = 14005451. / 335480064.;
    b1[1] = 0.;
    b1[2] = 0.;
    b1[3] = 0.;
    b1[4] = 0.;
    b1[5] = -59238493. / 1068277825.;
    b1[6] = 181606767. / 758867731.;
    b1[7] = 561292985. / 797845732.;
    b1[8] = -1041891430. / 1371343529.;
    b1[9] = 760417239. / 1151165299.;
    b1[10] = 118820643. / 751138087.;
    b1[11] = -528747749. / 2220607170;
    b1[12] = 0.25;

    b2 = new double[s];
    b2[0] = 13451932. / 455176623.;
    b2[1] = 0.;
    b2[2] = 0.;
    b2[3] = 0.;
    b2[4] = 0.;
    b2[5] = -808719846. / 976000145.;
    b2[6] = 1757004468. / 5645159321.;
    b2[7] = 656045339. / 265891186.;
    b2[8] = -3867574721. / 1518517206.;
    b2[9] = 465885868. / 322736535.;
    b2[10] = 53011238. / 667516719.;
    b2[11] = 2. / 45.;
    b2[12] = 0.;
